...
from audio_mir_eval.promotion import evaluate_promotion
...
def _report(
    ...
    return build_report(
        ...
        run={
            ...
            "model_sha256": "b" * 64,
            "analyzer_script_sha256": ("c" if arm == "omp-regularized" else "d") * 64,
            "experiment": {
...
def test_promotion_accepts_improved_frozen_candidate_with_held_out_evidence():
    ...
    assert decision["automation"] == {
        "eligible_tracks": 2,
        "reported_confidence_tracks": 2,
        "locked_tracks": 2,
        "coverage": 1.0,
        "false_locks": 0,
        "false_lock_rate": 0.0,
    ...
    assert "frozen_analyzer_script_sha256" not in {
