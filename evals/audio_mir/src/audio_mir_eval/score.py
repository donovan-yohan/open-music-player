...
_TEMPO_CHANGE_RELATIVE_THRESHOLD = 0.04
...
def _local_tempo_metrics(
    ...
    if reference_intervals == 0:
        return {
            ...
            **common,
            ...
            "tempo_change": {
                "threshold_relative": _TEMPO_CHANGE_RELATIVE_THRESHOLD,
    ...
    errors: list[float] = []
    tempo_change_true_positives = 0
    tempo_change_false_positives = 0
    tempo_change_false_negatives = 0
    tempo_change_true_negatives = 0
    comparable_change_boundaries = 0
    ...
    for reference_index in range(1, reference_intervals):
        estimate_before = matched_estimates.get(reference_index - 1)
        estimate_middle = matched_estimates.get(reference_index)
        estimate_after = matched_estimates.get(reference_index + 1)
        if estimate_before is None or estimate_middle is None or estimate_after is None:
            continue
        ...
        estimate_before_interval = float(
            estimated[estimate_middle] - estimated[estimate_before]
        )
        estimate_after_interval = float(
            estimated[estimate_after] - estimated[estimate_middle]
        )
        if (
            min(
                reference_before,
                reference_after,
                estimate_before_interval,
                estimate_after_interval,
            )
            <= 0
        ):
            continue
        comparable_change_boundaries += 1
        reference_changed = (
            abs(reference_after - reference_before) / reference_before
            > _TEMPO_CHANGE_RELATIVE_THRESHOLD
        )
        estimate_changed = (
            abs(estimate_after_interval - estimate_before_interval)
            / estimate_before_interval
            > _TEMPO_CHANGE_RELATIVE_THRESHOLD
        )
        if reference_changed and estimate_changed:
            tempo_change_true_positives += 1
        elif reference_changed:
            tempo_change_false_negatives += 1
        elif estimate_changed:
            tempo_change_false_positives += 1
        else:
            tempo_change_true_negatives += 1
    ...
    return {
        ...
        **common,
        ...
        "tempo_change": {
            "threshold_relative": _TEMPO_CHANGE_RELATIVE_THRESHOLD,
            "comparable_boundaries": comparable_change_boundaries,
            "true_positives": tempo_change_true_positives,
            "false_positives": tempo_change_false_positives,
            "false_negatives": tempo_change_false_negatives,
            "true_negatives": tempo_change_true_negatives,
            "false_positive_rate": round(
                tempo_change_false_positives
                / (tempo_change_false_positives + tempo_change_true_negatives),
                6,
            )
            if tempo_change_false_positives + tempo_change_true_negatives
            else None,
        },
    }
...
def score_track(manifest: dict[str, Any], prediction: dict[str, Any]) -> dict[str, Any]:
    result: dict[str, Any] = {
        ...
        "meter_known": "meter_beats" in manifest["reference"],
    ...
    for task, metric_names in _TASK_METRIC_NAMES.items():
        ...
        if task == "tempo":
            ...
            task_summary["local_tempo"] = {
                ...
                "tempo_change": {
                    "comparable_boundaries": sum(
                        metric["tempo_change"]["comparable_boundaries"]
                        for metric in local_tempo_evaluable
                    ),
                    "false_positives": sum(
                        metric["tempo_change"]["false_positives"]
                        for metric in local_tempo_evaluable
                    ),
                    "true_negatives": sum(
                        metric["tempo_change"]["true_negatives"]
                        for metric in local_tempo_evaluable
                    ),
                },
            }
            tempo_change = task_summary["local_tempo"]["tempo_change"]
            non_changes = (
                tempo_change["false_positives"] + tempo_change["true_negatives"]
            )
            tempo_change["false_positive_rate"] = (
                round(tempo_change["false_positives"] / non_changes, 6)
                if non_changes
                else None
            )
