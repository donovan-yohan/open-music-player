...
| AcousticBrainz import | `backend/cmd/acousticbrainz-import/`, `backend/internal/db/acousticbrainz_repository.go`, `backend/internal/discovery/acousticbrainz_hints.go`, `mb_acousticbrainz` table, `docs/ACOUSTICBRAINZ_IMPORT.md` | One-time CC0 dump loader keyed by recording MBID; read-time backfill fills only bpm/camelot fields the analyzer and user overrides left empty, provenance-tagged, never ground truth | `go -C backend test ./cmd/acousticbrainz-import/... ./internal/db -run AcousticBrainz`, `go -C backend test ./internal/discovery -run AcousticBrainz` |
