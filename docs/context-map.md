...
| Flutter client | `client/` | Android and web app, including a desktop-responsive web shell; native Windows/macOS runners are not present at the 2026-07-23 audit baseline | `scripts/test client`, `scripts/lint client`, `scripts/build client` |
| Product/platform audit | `docs/CROSS_PLATFORM_PLAYER_BENCHMARK_2026-07-23.md` | Dated Windows/macOS/Android competitor benchmark, staple-primitives matrix, and desktop-readiness recommendations; revalidate before treating absence claims as current | `scripts/lint delivery` |
