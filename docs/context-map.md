...
| Stem separation | `backend/cmd/stems-worker/`, `backend/internal/stems/` (incl. `worker.go`, the `stems:queue` consumer), `backend/Dockerfile` targets `stems-test`/`stems-runtime`, `docker-compose.local-low-memory.yml` service `stems` (profile `stems`), `docs/adr/0007-stems5-hybrid-v1-channel-set.md`, `docs/stems5-spec.md` | Opt-in htdemucs 4-stem separation plus the deterministic LR4-180 Hz kick/perc crossover (`stems5-hybrid-v1`); own Redis queue class, own `track_stems` table, never writes `track_analysis`. Off unless `STEMS_ENABLED=true` + the `stems` profile + Redis. `tracks.source_file_hash` is the replaced-audio detector that invalidates derived stem sets | `scripts/lint stems`, `scripts/test stems`, `go -C backend test ./cmd/stems-worker/... ./internal/stems/...`, `scripts/local-low-memory.sh start-stems` |
