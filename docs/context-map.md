...
| Stem separation (exploration) | `backend/cmd/stems-worker/`, `backend/internal/stems/`, `backend/Dockerfile` targets `stems-test`/`stems-runtime`, `docs/adr/0007-stems5-hybrid-v1-channel-set.md`, `docs/stems5-spec.md` | Opt-in htdemucs 4-stem separation plus the deterministic LR4-180 Hz kick/perc crossover (`stems5-hybrid-v1`); own Redis queue class, own `track_stems` table, never writes `track_analysis` | `scripts/lint stems`, `scripts/test stems`, `go -C backend test ./cmd/stems-worker/... ./internal/stems/...` |
