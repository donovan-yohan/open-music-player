...
| Stem separation | `backend/cmd/stems-worker/`, `backend/internal/stems/` (incl. `worker.go`, the `stems:queue` consumer), `backend/Dockerfile` targets `stems-test`/`stems-runtime`, `docker-compose.local-low-memory.yml` service `stems` (profile `stems`), `docs/adr/0007-stems5-hybrid-v1-channel-set.md`, `docs/stems5-spec.md` | Opt-in coherent `htdemucs_ft` four-stem separation through pinned `audio-separator==0.47.0`, plus deterministic LR4-180 Hz kick/perc crossover (`stems5-hybrid-v1`). The image bakes and hashes the provider wheel, local config, four weights, and UVR registry; runtime fails closed rather than downloading. `htdemucs_ft` is still a Demucs-family model, not a modern specialist route. Own Redis queue class, own `track_stems` table, never writes `track_analysis`. Off unless `STEMS_ENABLED=true` + the `stems` profile + Redis. | `scripts/lint stems`, `scripts/test stems`, `go -C backend test ./cmd/stems-worker/... ./internal/stems/...`, `scripts/local-low-memory.sh start-stems` |
| Audio MIR evals | `evals/audio_mir/`, `docs/AUDIO_MIR_EVALS.md`, `docs/evidence/audio-mir/`, `scripts/eval` | Local-only versioned benchmarks for tempo, beats/downbeats, and key; external catalog labels remain a separate reference class | `scripts/lint audio-mir`, `scripts/test audio-mir`, `scripts/eval audio-mir --help` |
| AcousticBrainz import | `backend/cmd/acousticbrainz-import/`, `scripts/acousticbrainz-import.sh`, `backend/internal/db/acousticbrainz_repository.go`, `backend/internal/discovery/acousticbrainz_hints.go`, `mb_acousticbrainz` table, `docs/ACOUSTICBRAINZ_IMPORT.md` (incl. "Running against a target", disk/time budget, verification queries, rollback) | One-time CC0 dump loader keyed by recording MBID; read-time backfill fills only bpm/camelot fields the analyzer and user overrides left empty, provenance-tagged, never ground truth; discovery candidates carry hint-only `ab_*` metadata on cache hit. The published dump layout does NOT match the loader's input: it must be projected to `mbid,bpm` / `mbid,key,scale` first. `scripts/acousticbrainz-import.sh` wraps fetch/project/shard/load/verify and enforces the pinned dump sha256s. The only table the *import* writes is `mb_acousticbrainz` (rollback `TRUNCATE mb_acousticbrainz;`), but the loader's startup `database.Migrate()` additionally normalizes existing `research_*` rows, which neither that rollback nor the before/after census covers. Budget from measured numbers: 7,564,449 projected rows, ~309 upserts/s, ~7 h for a full 16-shard load | `go -C backend test ./cmd/acousticbrainz-import/... ./internal/db -run AcousticBrainz`, `go -C backend test ./internal/discovery -run AcousticBrainz` |
...
| Harmonic DJ lineup block | `backend/internal/api/dj_lineup_harmonic.go`, `backend/internal/api/dj_lineup_handlers.go`, `backend/internal/db/nearby_tracks_repository.go`, `docs/adr/0008-harmonic-lineup-candidate-composition.md` | Default-off `ENABLE_HARMONIC_LINEUP` block that leads the DJ lineup with tracks mixing cleanly out of the client-supplied queue-tail anchor; reuses the shipped nearby+affinity read, adds no SQL, and is byte-identical to the themed lineup while off | `go -C backend test ./internal/api -run Lineup` |
...
| Protected dogfood database | `backend/internal/db/environment.go`, `backend/internal/db/testguard.go`, `scripts/local-low-memory.sh`, `scripts/dogfood-backup.sh`, `scripts/tests/`, `deploy/README.md` | Single-row `omp_environment` marker plus the refusals it drives: truncating integration-test helpers stop on a protected DSN port or marker, `clean` refuses `down -v` on a protected compose project, and `dogfood-backup.sh` handles `pg_dump -Fc` retention and guarded restore. Escape hatches are `OMP_ALLOW_PROTECTED_DB_TESTS`, `OMP_ALLOW_PROTECTED_STACK_TEARDOWN`, `OMP_ALLOW_PROTECTED_DB_RESTORE` | `go -C backend test ./internal/db -run Protected`, `scripts/lint delivery` |
...
- Guardrail: default crossfade lives on `MixSession` beside
  `transitionSnapMode`; settings reach the engine only through
  `PlaybackState.applyAudioDefaults`. Never add a second fade channel.
