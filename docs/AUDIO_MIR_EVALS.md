...
baseline intent; it is not a substitute for the report's concrete hashes. The
value must be the evaluator's canonical SHA-256 freeze payload (manifest,
per-track audio, model, run/scorer repository identities, analyzer metadata,
experiment ID, and factor; never arm labels). An arbitrary shared label fails
promotion.
The promotion command also emits a canonical, content-addressed evidence packet:
stdlib JSON with sorted keys, compact separators, and SHA-256. It binds the
manifest and per-track audio hashes, baseline/candidate model and repository
identities, policy, reports, and exact decision. This small whole-packet hash is
intentional: evaluator artifacts are bounded, so there is no Merkle, WAL, or
replay requirement to justify extra protocol complexity.
...
```json
{
  ...
  "downbeats": {
    ...
    "minimum_paired_phase_precision_lower_ci": 0.01,
    "minimum_paired_cmlc_lower_ci": 0.00
  },
  "beats": {
    "maximum_f_measure_regression": 0.01,
    "maximum_cemgil_regression": 0.01,
    ...
    "maximum_tempo_change_false_positive_rate_increase": 0.01
  },
  "paired": {
    "minimum_tracks": 20,
    "bootstrap_resamples": 10000
  },
  "required_holdout_strata": ["steady_edm", "breakdown", "variable_tempo"],
  "resources": {
  ...
  "automation": {
    ...
    "calibration_evidence_sha256": "<canonical-calibration-only-evidence-sha256>",
...
scripts/eval audio-mir promote \
  ...
  --evidence-output /tmp/downbeat-promotion-evidence.json
```
...
calibration corpus; the policy must bind the canonical SHA-256 of its
ground-truth calibration rows, which intentionally excludes all holdout rows.
The promotion command then checks calibration and sealed holdout false-lock risk
at that selected threshold.
...
local-tempo including tempo-change false-positive rate, zero-infrastructure-error,
runtime/RSS, calibration, coverage, required holdout strata, deterministic
paired per-track phase/CMLc bootstrap lower confidence bounds, unknown-meter and
unknown-phase abstention, and false-lock gate passes. It writes decision and
evidence artifacts either way.
