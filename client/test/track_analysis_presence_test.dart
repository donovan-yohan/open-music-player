...
void main() {
  group('TrackAnalysis override presence', () {
    ...
    test('the analysis parser preserves an explicit clear', () async {
      final ApiClient api = mockQueueApiClient((request) async {
    ...
    test('the analysis parser keeps an omitted override absent', () async {
      final ApiClient api = mockQueueApiClient((request) async {
        return http.Response(
          jsonEncode({'status': 'analyzed'}),
          200,
          headers: {'content-type': 'application/json'},
        );
      });
      ...
      expect(analysis.overridesPresent, isFalse);
      expect(analysis.toJson(), isNot(contains('overrides')));
    });
