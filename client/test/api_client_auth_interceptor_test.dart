...
void main() {
  ...
  test(
    'sends the access token to logout without refreshing a logout 401',
    () async {
      final storage = _MemoryTokenStorage(
        accessToken: 'expired-access-token',
        refreshToken: 'valid-refresh-token',
      ...
      await expectLater(api.post('/auth/logout'), throwsA(isA<DioException>()));
  ...
  test(
    'refreshes a non-auth 401 once and retries with the fresh access token',
    () async {
      final storage = _MemoryTokenStorage(
        accessToken: 'expired-access-token',
        refreshToken: 'valid-refresh-token',
