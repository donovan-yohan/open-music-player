...
void main() {
  ...
  group('djDeckEntryHintFor', () {
    Future<String?> readHint(
      ...
      if (playback != null) {
        probe = legacy_provider.ChangeNotifierProvider<PlaybackState>.value(
          value: playback,
          child: probe,
        );
      }
    ...
    // ADR 0012: the hint must judge the *playing* track from playback truth.
    // It previously read `QueueProvider.currentTrack`, whose `currentPosition`
    // the backend never advances, so an empty import queue made the hint claim
    // there was no current track while audio was plainly playing.
    testWidgets(
        'an empty import queue does not hide the hint while audio is playing',
        (tester) async {
      final playback = TestPlaybackState(
        queue: [playbackMediaItem(4242)],
        currentIndex: 0,
      );
      ...
      expect(
        await readHint(tester, playback: playback, downloads: downloads),
    ...
    testWidgets('a row that cannot be downloaded advertises nothing',
      ...
      final playback = TestPlaybackState(
        queue: [
          playbackMediaItem(0, title: 'Source-backed row'),
        ],
        currentIndex: 0,
      );
      addTearDown(playback.dispose);
