...
func newPlayEventTestDB(t *testing.T) (*DB, context.Context) {
	...
	return newGuardedTestDB(t,
		"set OMP_POSTGRES_TEST_DSN, QA_DATABASE_URL, or DATABASE_URL to run Postgres play-event integration tests",
		"TRUNCATE TABLE play_events, user_library, tracks, users RESTART IDENTITY CASCADE")
}
