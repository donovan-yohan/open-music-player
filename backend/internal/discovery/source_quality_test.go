...
func TestSourceQualityPrefersOfficialYouTubeMusicAudioForReportedExamples(t *testing.T) {
	tests := []struct {
		name    string
		query   string
		audioID string
		...
		{name: "Speakerphone", query: "Kylie Minogue Speakerphone", audioID: "ipLo9enSiB4", videoID: "WcHW89jq1kk", artist: "Kylie Minogue", track: "Speakerphone", album: "X", label: "Parlophone Records", audioMs: 205000, videoMs: 226000},
		{name: "iPod Touch", query: "Ninajirachi iPod Touch", audioID: "xtRVa4kOBt4", videoID: "MxekyGtqcNE", artist: "Ninajirachi", track: "iPod Touch", album: "I Love My Computer", label: "NLV Records", audioMs: 211000, videoMs: 247000},
	}

	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			audio := Candidate{
				CandidateID: "youtube:" + tc.audioID, Provider: "youtube", SourceID: tc.audioID,
				SourceURL: "https://www.youtube.com/watch?v=" + tc.audioID, Title: tc.track, Artist: tc.artist,
				Uploader: tc.artist + " - Topic", DurationMs: tc.audioMs, Downloadable: true,
				Metadata: map[string]interface{}{"description": "Provided to YouTube by " + tc.label + "\\nAuto-generated by YouTube", "track": tc.track, "artist": tc.artist, "album": tc.album, "label": tc.label, "release_date": "2025-01-01", "discoverySurface": "youtube_music_songs"},
			}
			video := Candidate{
				CandidateID: "youtube:" + tc.videoID, Provider: "youtube", SourceID: tc.videoID,
				SourceURL: "https://www.youtube.com/watch?v=" + tc.videoID, Title: tc.artist + " - " + tc.track + " (Official Video)",
				Uploader: tc.artist, DurationMs: tc.videoMs, Downloadable: true,
				Metadata: map[string]interface{}{"description": "Official Video\\nDirected by Example Director\\nProduction credits"},
			}

			ranked := rankSourceCandidates(tc.query, []Candidate{video, audio})
			if got := ranked[0].SourceID; got != tc.audioID {
				t.Fatalf("top source = %s, want official YouTube Music audio %s", got, tc.audioID)
			}
			...
			if audioQuality.Classification != SourceQualityOfficialAudio || audioQuality.Recommendation != SourceQualityPreferred {
				t.Fatalf("audio quality = %#v, want preferred official_audio", audioQuality)
			}
			if videoQuality.Classification != SourceQualityMusicVideo || videoQuality.Score >= audioQuality.Score {
				t.Fatalf("video quality = %#v, want lower music_video than %#v", videoQuality, audioQuality)
			}
		})
	}
}
...
func TestSourceQualityFlatYouTubeMusicSongsDoNotInferOfficialAudio(t *testing.T) {
	tests := []struct {
		...
		{name: "Speakerphone", query: "Kylie Minogue Speakerphone", audioID: "ipLo9enSiB4", videoID: "WcHW89jq1kk", artist: "Kylie Minogue", track: "Speakerphone", audioMs: 205000, videoMs: 226000},
		{name: "iPod Touch", query: "Ninajirachi iPod Touch", audioID: "xtRVa4kOBt4", videoID: "MxekyGtqcNE", artist: "Ninajirachi", track: "iPod Touch", audioMs: 211000, videoMs: 247000},
	}
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			audio := Candidate{
				CandidateID: "youtube:" + tc.audioID, Provider: "youtube", SourceID: tc.audioID,
				SourceURL: "https://www.youtube.com/watch?v=" + tc.audioID, Title: tc.track,
				Uploader: tc.artist + " - Topic", DurationMs: tc.audioMs, Downloadable: true,
			...
			ranked := rankSourceCandidates(tc.query, []Candidate{video, audio})
			if got := ranked[0].SourceID; got != tc.audioID {
				t.Fatalf("top source = %s, want flat YouTube Music audio %s", got, tc.audioID)
			}
			audioQuality := sourceQualityFromMetadata(t, ranked[0].Metadata)
			videoQuality := sourceQualityFromMetadata(t, ranked[1].Metadata)
